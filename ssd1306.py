"""MicroPython SSD1306 OLED monochrom display driver."""
...
class Display(object):
    """Serial and I2C interface for SSD1306 monochrome OLED display.
    ...
    def draw_line(self, x1, y1, x2, y2, invert=False):
        ...
        if y1 == y2:
            if x1 > x2:
                x1, x2 = x2, x1
            self.draw_hline(x1, y1, x2 - x1, invert)
            return
        # Check for vertical line
        if x1 == x2:
            if y1 > y2:
                y1, y2 = y2, y1
            self.draw_vline(x1, y1, y2 - y1, invert)
            return
